    ...
    <>
      ...
      <DirectoryHero
        ...
        stat={
          <span className="inline-flex min-h-12 items-center gap-2.5 rounded-full border border-white/90 bg-white/[0.86] px-4 text-sm font-extrabold text-[#4f5b67] shadow-[0_14px_40px_rgb(16_30_36_/_0.07)]">
            <UsersIcon />
            {tFormat("doctors.resultsCount", { count: String(doctors.meta.total) })}
          </span>
        }
        filters={
          <DoctorsFilterBar
      ...
      {doctors.data.length === 0 ? (
        <section className="space-y-6">
          <div>
            <h2 className="text-2xl font-black tracking-tight">{t("doctors.resultsTitle")}</h2>
        ...
        <DoctorsResultsSection doctors={doctors.data} total={doctors.meta.total} />
      )}
...
function StethoscopeIcon() {
  return (
    <svg className="h-4 w-4 text-accent" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden>
      <path d="M11 2v2M6 7a4 4 0 018 0M6 7v2a6 6 0 0012 0V7M6 7H4" strokeLinecap="round" />
    </svg>
  );
}
...
function UsersIcon() {
  return (
    <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden>
      <path d="M16 21v-2a4 4 0 00-4-4H6a4 4 0 00-4 4v2M9 7a4 4 0 100-8 4 4 0 000 8zM22 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75" strokeLinecap="round" />
    </svg>
  );
}
