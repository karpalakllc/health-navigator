...
export function PharmacySidebarContact({ pharmacy }: { pharmacy: PharmacyDetail }) {
  const telHref = pharmacy.phone ? `tel:${pharmacy.phone.replace(/\s+/g, "")}` : null;
    ...
    <aside className="space-y-4 lg:sticky lg:top-28 lg:self-start">
      <div className="content-card rounded-[1.875rem] p-[22px]">
        <h2 className="text-[1.7rem] font-black tracking-tight text-foreground">
          {t("pharmacies.quickActionsTitle")}
        </h2>
        ...
        {[pharmacy.address, pharmacy.city].filter(Boolean).length > 0 ? (
          <address className="mt-[18px] not-italic text-sm leading-relaxed text-[#59656f]">
            {pharmacy.address ? <div>{pharmacy.address}</div> : null}
            {pharmacy.city ? <div>{pharmacy.city}</div> : null}
          </address>
        ) : null}
        ...
        <div className="mt-[18px] rounded-[1.375rem] border border-primary/15 bg-[#fff1f1] p-4">
          <div className="mb-2.5 flex items-center gap-2.5 font-extrabold text-[#313d47]">
            ...
            {t("pharmacies.sidebarDisclaimerTitle")}
          </div>
